...
export default function HomePage() {
  ...
  const allBranchKeys = Array.from(new Set([...refBranches.map(b => b.valeur), ...filmBranches]))

  const getBranchColor = (key: string, idx: number): { bg: string; text: string } => {
    const ref = refBranches.find(b => b.valeur === key)
    if (ref?.couleur) {
      const hex = ref.couleur.replace('#','')
      const r = parseInt(hex.slice(0,2),16), g = parseInt(hex.slice(2,4),16), b = parseInt(hex.slice(4,6),16)
      const lum = (0.299*r + 0.587*g + 0.114*b) / 255
      return { bg: ref.couleur, text: lum > 0.5 ? '#1a1a18' : '#fff' }
    }
    const i = idx % DEFAULT_COLORS.length
    return { bg: DEFAULT_COLORS[i], text: DEFAULT_TEXTS[i] }
  }

  const branchColorMap: Record<string, { bg: string; text: string }> = {}
  allBranchKeys.forEach((k, i) => { branchColorMap[k] = getBranchColor(k, i) })

  // Couleur "commun" = gris neutre pour les films sans branche spécifique
  const communColor = { bg: '#E0DDD8', text: '#5F5E5A' }
  ...
  const getPoster = (film: Film): string => {
    if (film.poster_url) return film.poster_url
    const info = getVideoInfo(film.fichier_url || '')
        ...
        <div className="card" style={{ padding: '1rem', position: 'relative', overflow: 'visible' }}>
          <div className="frise-outer" style={{ position: 'relative' }}>
            <div style={{ position: 'relative', height: 28, marginBottom: 4 }}>
              {tickYears.map(y => (
                <div key={y} style={{ position: 'absolute', left: ((y - yearMin) / span * 100).toFixed(2) + '%', transform: 'translateX(-50%)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingBottom: 8, overflow: 'hidden' }}>
              {films.map(film => {
                ...
                const color      = mainBranch ? (branchColorMap[mainBranch] || communColor) : communColor
            ...
            <div style={{ display: 'flex', gap: 16, paddingTop: 8, borderTop: '1px solid var(--gray-border)', flexWrap: 'wrap' }}>
              {allBranchKeys.map(k => (
                <div key={k} className="flex items-center gap-1">
                  <div style={{ width: 10, height: 10, borderRadius: 2, background: branchColorMap[k]?.bg || communColor.bg }} />
                  <span className="text-sm text-muted">{k}</span>
                </div>
              ))}
              <div className="flex items-center gap-1">
                <div style={{ width: 10, height: 10, borderRadius: 2, background: communColor.bg }} />
                <span className="text-sm text-muted">Commun / non classé</span>
              </div>
            </div>
            ...
            .map(film => {
              ...
              return (
                <Link key={film.id} href={`/film/${film.id}`}>
                  <div className="card" style={{ cursor: 'pointer', transition: 'border-color 0.1s', padding: 0, overflow: 'hidden' }}
                  ...
                  >
                    {/* Poster */}
                    <div style={{ aspectRatio: '16/9', background: color.bg + '55', position: 'relative', overflow: 'hidden' }}>
                      {poster
                        ? <img src={poster} alt={film.titre} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, color: color.bg }}>▶</div>
                      }
                      {/* Badge couverture */}
                      {(film.couverture || 0) >= 100 && (
                        <div style={{ position: 'absolute', top: 6, right: 6, background: 'rgba(29,158,117,0.9)', color: '#fff', fontSize: 10, padding: '2px 7px', borderRadius: 99, fontWeight: 500 }}>✓</div>
                      )}
                    </div>

                    <div style={{ padding: '10px 12px 12px' }}>
                      <div style={{ fontWeight: 500, fontSize: 13, marginBottom: 3 }}>{film.titre}</div>
                      <div className="text-sm text-muted" style={{ marginBottom: 8 }}>
                        {film.date_label || (film.annee_fin && film.annee !== film.annee_fin ? `${film.annee}–${film.annee_fin}` : film.annee)}
                      </div>
                      {(film.couverture || 0) > 0 && (film.couverture || 0) < 100 && (
                        <div className="flex items-center gap-2 mb-2">
                          <div className="progress-wrap flex-1">
                            <div className="progress-fill" style={{ width: (film.couverture || 0) + '%' }} />
                          </div>
                          <span className="text-sm text-muted">{film.couverture}%</span>
                        </div>
                      )}
                      {/* Tags branches avec couleur du référentiel */}
                      <div className="flex wrap gap-1">
                        {branches.length > 0
                          ? branches.map(b => {
                              const c = branchColorMap[b] || communColor
                              return (
                                <span key={b} style={{ fontSize: 11, padding: '2px 8px', borderRadius: 99, background: c.bg + '33', color: c.text, border: `1px solid ${c.bg}`, fontWeight: 500 }}>
                                  {b}
                                </span>
                              )
                            })
                          : <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 99, background: communColor.bg, color: communColor.text, fontWeight: 500 }}>Commun</span>
                        }
                      </div>
                    </div>
                  </div>
